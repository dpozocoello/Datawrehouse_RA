-- [v1.3 EXPERT] Enhanced Hierarchy Resolution with INEC Validation
-- Fase A: Normalización contra el Catálogo Nacional y Corrección de Anomalías
...
INSERT INTO dw.dim_area (
    ...
    pivoted_geo AS (
        SELECT area_id,
            ...
            -- INEC Codes for validation [v1.3]
            MAX(
                CASE
    ...
    resolved_geo AS (
        SELECT area_id,
            -- Logic based on ECUADOR location + INEC Validation [v1.3]
            CASE
    ...
    expert_geo AS (
        SELECT rg.area_id,
            -- Normalización INEC: Si el código es '90' y el nombre sugiere Guayas, o no está en el catálogo, se marca N/A o se corrige.
            -- El Plan Experto pide filtrar contra ref.inec_dpa_2024.
            COALESCE(
                i.nombre_provincia,
                CASE
                    WHEN rg.inec_prov = '90'
                    AND rg.prov_raw ILIKE '%SALITRE%' THEN 'GUAYAS' -- Corrección específica hallazgo Salitre
                    ELSE 'N/A'
                END
            ) as prov_final,
            rg.cant,
...
SELECT s.area_id,
    ...
    END
FROM stg.suia_areas_bi s
    LEFT JOIN expert_geo eg ON s.area_id = eg.area_id ON CONFLICT (id_area) DO
UPDATE
...
END IF;
RAISE NOTICE 'Dimensión Areas (v1.3 Expert) cargada exitosamente con validación INEC';
END;
$$ LANGUAGE plpgsql;