/*
 * Fix: Captura de Biodiversidad Multicanal (v1.4.1)
 * Autor: Antigravity AI
 * Descripción: Refactorización de SPs para incluir Patrimonio Forestal, Zonas Intangibles y Bosques Protectores.
 */
-- [REF 1] Refactorización de suia_iii.sp_coa_bi()
...
FROM (
        ...
        FROM (
                SELECT DISTINCT a.pren_code AS "CÓDIGO PROYECTO",
                    ...
                    ar.area_id "ID AREA",
                    -- [FIX] Se amplía para capturar laye_id 2, 3, 4, 11
                    (
                        string_agg(
                            concat(
                ...
                FROM suia_iii.projects_environmental_licensing a
                    ...
                    left JOIN geographical_locations gl6 ON gl5.gelo_parent_id = gl6.gelo_id -- [FIX] Cambio laye_id = 3 por IN (2, 3, 4, 11)
...
END;
$function$;
-- [REF 2] Refactorización de coa_mae.sp_rcoa_bi()
CREATE OR REPLACE FUNCTION coa_mae.sp_rcoa_bi() RETURNS void LANGUAGE plpgsql AS $function$ BEGIN truncate table coa_mae.tmp_rcoa_bi;
        ...
        FROM (
                ...
                FROM (
                        SELECT DISTINCT a.prco_cua AS "CÓDIGO PROYECTO",
                            ...
                            -- [FIX] Cambio laye_id = 3 por IN (2, 3, 4, 11) y agrego separador
                            (
                                string_agg(
                                    concat(
                        ...
                        FROM coa_mae.project_licencing_coa a
                            ...
                            LEFT JOIN areas_types at ON at.arty_id = ar.arty_id
                            LEFT JOIN areas z ON ar.area_parent_id = z.area_id
                            LEFT JOIN zones z1 ON z1.zone_id = z.zone_id -- [FIX] Cambio laye_id = 3 por IN (2, 3, 4, 11)
