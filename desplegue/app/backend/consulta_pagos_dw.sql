-- ==============================================================================
-- CONSULTA ADAPTADA AL DATA WAREHOUSE (dw_reg_v1) - SIN DBLINK
-- Datos de pagos integrados localmente via dw.fact_pago + dw.dim_pago
-- ==============================================================================
-- Servidor Local: dw_reg_v1 (esquemas dw.* y stg.*)
-- Ya NO requiere dblink ni conexion remota a 172.16.0.226
-- Los pagos se cargan via ETL: TRX_07 (JBPM) + TRX_08 (SUIA)
-- ==============================================================================
WITH local_projects AS (
    -- Proyectos filtrados desde el Data Warehouse (modelo estrella)
    SELECT DISTINCT ON (dp.codigo_proyecto) dp.codigo_proyecto AS "CÓDIGO PROYECTO",
        ...
        f.tarea AS "TAREA",
        de.estado_tramite AS "ESTADO TAREA",
...
pagos_proyecto AS (
    -- Pagos locales desde dw.fact_pago (ya sin dblink)
    -- Incluye pagos de ambos origenes: JBPM y SUIA_RCOA
...
) -- Resultado final: Proyectos del DW con pagos locales (sin dblink)
SELECT lp.*,
