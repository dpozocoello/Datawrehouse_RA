...
def assemble_sp_v1_4():
    ...
    for _, row in df.iterrows():
        ...
        if p != 'nan':
            prov_blocks += f"        WHEN s.area_id = {int(row['id_area'])} THEN '{p}' -- {n}\n"
        if c != 'nan':
            cant_blocks += f"        WHEN s.area_id = {int(row['id_area'])} THEN '{c}'\n"

    sp_template = f"""-- [v1.4 EXPERT] Full Political Geography Normalization
-- Fase A: Inferencia de Geografía Política para áreas sin gelo_id
...
SELECT area_id,
    ...
    zona_str,
    area_campus,
    -- [v1.4 EXPERT FALLBACK] Complete Political Geography Mapping
    CASE 
        WHEN prov_res = 'N/A' THEN
            CASE 
{prov_blocks}
                ELSE 'N/A'
            END
...
RAISE NOTICE 'Dimensión Areas (v1.4 Expert Full Geo) cargada exitosamente';
END;
$$ LANGUAGE plpgsql;
"""
    with open("f:/Datawrehouse_RA/sp_expert_dim_area_v1_4.sql", "w", encoding="utf-8") as f:
        f.write(sp_template)
    print("SP v1.4 assembled successfully.")
