-- Final fix for string length issues in fact_regularizacion
-- 1. Drop dependent objects
DROP MATERIALIZED VIEW IF EXISTS dw.mv_dashboard_regularizacion;
DROP VIEW IF EXISTS dw.v_dashboard_regularizacion;
-- 2. Alter column types to TEXT for safety
ALTER TABLE dw.fact_regularizacion
ALTER COLUMN interseccion_snap TYPE TEXT;
ALTER TABLE dw.fact_regularizacion
ALTER COLUMN areas_protegidas TYPE TEXT;
-- 3. Recreate View with JOIN to dim_area
