...
def assemble_sp_v1_4_fixed():
    ...
    for _, row in df.iterrows():
        ...
        if p != 'nan':
            # Use 'area_id' instead of 's.area_id' because it's in the final SELECT scope
            prov_blocks += f"            WHEN f.area_id = {int(row['id_area'])} THEN '{p}' -- {n}\n"
        if c != 'nan':
            cant_blocks += f"            WHEN f.area_id = {int(row['id_area'])} THEN '{c}'\n"

    sp_template = f"""-- [v1.4 EXPERT] Full Political Geography Normalization
-- [FIXED] Updated aliases and scope for CASE statements
...
RAISE NOTICE 'Dimensión Areas (v1.4 Expert Full Geo [FIXED]) cargada exitosamente';
END;
$$ LANGUAGE plpgsql;
"""
    with open("f:/Datawrehouse_RA/sp_expert_dim_area_v1_4.sql", "w", encoding="utf-8") as f:
        f.write(sp_template)
    print("SP v1.4 assembled successfully with FIXED aliases.")

if __name__ == "__main__":
    assemble_sp_v1_4_fixed()
