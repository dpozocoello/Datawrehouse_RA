...
%PSQL% -h %DB_HOST% -U %DB_USER% -d %DB_NAME% -f "f:\Datawrehouse_RA\etl_carga_datos.sql"
echo [INFO] Carga Base completada.

echo [2/4] Ejecutando Transformaciones Globales v2 (Bridge Table, Area Responsable, Secuencias)...
%PSQL% -h %DB_HOST% -U %DB_USER% -d %DB_NAME% -f "f:\Datawrehouse_RA\v2\etl_carga_datos_v2.sql"
if %ERRORLEVEL% NEQ 0 (
...
%PSQL% -h %DB_HOST% -U %DB_USER% -d %DB_NAME% -f "f:\Datawrehouse_RA\v3\etl_carga_datos_v3.sql"
if %ERRORLEVEL% NEQ 0 (
...
%PSQL% -h %DB_HOST% -U %DB_USER% -d %DB_NAME% -f "f:\Datawrehouse_RA\etl_intersection_load.sql"
if %ERRORLEVEL% NEQ 0 (
