...
SELECT
    ...
    description,
    project_id,
    NULLIF(REPLACE(retired_value::text,  ',', '.'), '')::double precision AS retired_value,
    sender_ip,
    tramit_number,
    update_date,
    NULLIF(REPLACE(value_updated::text,  ',', '.'), '')::double precision AS value_updated,
    online_payment_id,
    enabled,
    user_create,
    user_modification,
    ...
    reactivate,
    observations,
    NULLIF(REPLACE(retired_value_1::text, ',', '.'), '')::double precision AS retired_value_1
...
# ==============================================================================
# TRX_10B: Ingesta Catálogo de Tipos de Área (v2.0)
# ==============================================================================
# Origen: suia_enlisy → public.areas_types
# Destino: stg.suia_areas_types_bi
# Descripción: Extrae los tipos de área (Oficina Técnica, Dirección Zonal,
#   Dirección Provincial, etc.) necesarios para clasificar dim_area
#   en el dashboard de regularización ambiental.
# ==============================================================================
QUERY_TRX_10B = """
SELECT
    arty_id,
    ...
    arty_abbreviation,
    arty_status
FROM public.areas_types
"""

@medir_tiempo("TRX_10B_AREAS_TYPES")
def ejecutar_trx_10b() -> int:
    """Extrae catálogo de tipos de área desde suia_enlisy y carga en stg.suia_areas_types_bi."""
    return extract_to_staging(
        origen_params=CONN_SUIA_ENLISY,
        destino_params=CONN_DWH_LOCAL,
        query_origen=QUERY_TRX_10B,
        tabla_destino="stg.suia_areas_types_bi",
        paso_nombre="TRX_10B_AREAS_TYPES",
        batch_size=BATCH_SIZE
    )
