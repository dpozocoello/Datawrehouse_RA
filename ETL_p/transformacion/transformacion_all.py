...
from connections import execute_sp, execute_sql, get_connection
...
# ==============================================================================
# SETUP REFERENCE DATA (equivalente a "Setup Reference Data" de Pentaho)
# ==============================================================================
# Equivalente Pentaho: Setup Reference Data (SQL file: setup_reference_data_v1_4.sql)
#
# Descripción:
#   Inicializa los registros centinela SK=0 en todas las dimensiones del DWH.
#   Estos registros "DESCONOCIDO / N/A" son requeridos por las FK de fact tables
#   cuando un proyecto no puede resolverse a una dimensión concreta.
#   También crea el esquema ref y carga el catálogo INEC 2024.
#
#   SIN este paso, sp_carga_hechos() falla con FK violation en dim_area(sk_area=0).
#
# Dependencias:
#   - Debe ejecutarse ANTES de cualquier SP de transformación
#   - Idempotente: usa ON CONFLICT DO NOTHING en todos los INSERTs
# ==============================================================================
_SETUP_SQL_PATH = pathlib.Path(PROJECT_ROOT) / "setup_reference_data_v1_4.sql"

@medir_tiempo("SETUP_REFERENCE_DATA")
def setup_reference_data():
    """Inserta registros SK=0 en todas las dimensiones (setup idempotente)."""
    sql = _SETUP_SQL_PATH.read_text(encoding="utf-8")
    with get_connection(CONN_DWH_LOCAL) as conn:
        with conn.cursor() as cur:
            cur.execute(sql)
        conn.commit()
    logger.info("[SETUP_REFERENCE_DATA] Registros SK=0 y catálogo INEC inicializados.")
...
WITH desc_saldos AS (
    SELECT
        ...
        COALESCE(
            ...
            (
                oph.value_updated::numeric
                + COALESCE(oph.retired_value, 0)::numeric
            )
        ) AS saldo_anterior,
...
# ==============================================================================
# PASO 16: Jerarquía Recursiva dim_area (v3.0)
# ==============================================================================
# Stored Procedure: dw.sp_carga_jerarquia_area()
#
# Descripción:
#   Materializa la jerarquía recursiva de public.areas en dim_area:
#     - nivel_jerarquico (1/2/3)
#     - l1_id / l1_nombre / l1_siglas_tipo  (ancestro raíz)
#     - l2_id / l2_nombre / l2_siglas_tipo  (ancestro nivel 2)
#     - ruta_ids / ruta_nombres             (breadcrumb)
#     - es_hoja                             (nodo terminal)
#   Y puebla dw.bridge_area_jerarquia con todos los pares ancestro↔descendiente.
#
# Dependencias:
#   - Script 04 y 05 aplicados en DBeaver
#   - paso 15 (SP_CARGA_DIM_AREA) completado
# ==============================================================================
@medir_tiempo("SP_JERARQUIA_AREA")
def sp_carga_jerarquia_area():
    """Ejecuta dw.sp_carga_jerarquia_area() — Materaliza jerarquía recursiva y bridge table."""
    execute_sp(CONN_DWH_LOCAL, "SELECT dw.sp_carga_jerarquia_area()", "SP_JERARQUIA_AREA")
