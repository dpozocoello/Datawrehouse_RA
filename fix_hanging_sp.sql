-- ==============================================================================
-- SCRIPT DE OPTIMIZACION Y LOGGING PARA dw.sp_carga_fact_pago()
-- ==============================================================================
-- 1. Crear índice faltante para acelerar la Parte B (Asociación Indirecta)
CREATE INDEX IF NOT EXISTS idx_fact_regularizacion_proponente ON dw.fact_regularizacion(sk_proponente);
-- 2. Versión mejorada del SP con logging detallado
...
RAISE NOTICE '[%] PASO 0: Creando tabla temporal de pagos únicos...',
clock_timestamp();
...
RAISE NOTICE '[%] PASO 0 Completado. Pagos únicos a procesar: %',
clock_timestamp(),
(
    SELECT COUNT(*)
    FROM tmp_jbpm_unicos
);
-- ══════════════════════════════════════════════════════════════════
...
-- ══════════════════════════════════════════════════════════════════
RAISE NOTICE '[%] PARTE B: Iniciando asociación indirecta por Proponente (esto puede tardar)...',
clock_timestamp();
INSERT INTO dw.fact_pago (
        sk_proyecto,
        sk_pago,
