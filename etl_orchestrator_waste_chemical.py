...
def execute_etl():
    try:
        ...
        # 1. Leer los scripts SQL con ruta absoluta
        base_dir = os.path.dirname(os.path.abspath(__file__))
        
        with open(os.path.join(base_dir, 'etl_waste_chemical_extract.sql'), 'r', encoding='utf-8') as f:
            extract_sql = f.read()
            
        with open(os.path.join(base_dir, 'etl_waste_chemical_load.sql'), 'r', encoding='utf-8') as f:
            load_sql = f.read()
